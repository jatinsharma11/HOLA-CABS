...
#define lat1d 12.9611159                         // user lat

// Longitude of customer who needs a cab.
#define lon1d 77.6362214                         // user long

#define pi 3.14159265358979323846               // degree se radian
#define earth_radius 6371.0                     // GCD Formula - radius

ifstream customer_list ("customers.json");        
ofstream out ("answer.json");                       

// Function to convert degree to radian.
double degtorad(double deg)                          // 2*pi /180
{
	return ( deg * pi / 180);
}
...
{				
	...
	lat1 = degtorad(lat1d);   //U
	lon1 = degtorad(lon1d);   //U
	lat2 = degtorad(lat2d);   //D
	lon2 = degtorad(lon2d);   //D

	delta_lon = lon2 - lon1;
	
	// great circle distance formula.
	central_ang = acos ( sin(lat1) *                           //GCD formula
				sin(lat2) + cos(lat1) *
...
struct json                                                //Fetching
{
	...
	*/
	...
	char latitude_as_string[1000],  // string type ka file me store 
		longitude_as_string[1000],   // string type ka file me store hoga
	...
	{					
		...
		customer_list.close();   // Fine bnai thi ussko close 
		
		// closing stream of answer's file.
		out.close();  // answer naam ki file .
	}
};
